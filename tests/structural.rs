...
fn temp() -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("solomon-structural-{}-{id}", std::process::id()))
}
...
const PROGRAMS: &[(&str, &str)] = &[
    ("return", "return 42;"),
    ("arith", r#""%d\n", 2 + 3 * 4 - 5;"#),
    ("printf", r#""x=%d hex=%x s=%s\n", 42, 255, "hi";"#),
    ("float", r#""%.2f %9.3f %f\n", 2.5, 3.14159, -0.001;"#),
    (
        "loop",
        "I64 i, s = 0; for (i = 0; i < 10; i++) s += i; \"%d\\n\", s;",
    ),
    ("call", "I64 Sq(I64 x){ return x*x; } \"%d\\n\", Sq(6) + 6;"),
    (
        "class",
        "class P { I64 x; I64 y; } P p; p.x = 3; p.y = 4; \"%d\\n\", p.x + p.y;",
    ),
    (
        "string",
        "#include <string.hc>\nU8 *b = MAlloc(32); StrCpy(b, \"hi\"); StrCat(b, \"!\"); \"%s %d\\n\", b, StrLen(b);",
    ),
    (
        "global_bss",
        "I64 g = 5; U0 Bump(){ g++; } Bump(); Bump(); \"%d\\n\", g;",
    ),
    (
        "exceptions",
        "U0 Boom(){ throw(7); } try { Boom(); } catch { \"caught %d\\n\", Fs->except_ch; }",
    ),
];
...
fn macho_object_has_text_and_symbols() {
    ...
    let obj = macho(&checked("return 42;"));
...
fn freestanding_elf_is_self_contained() {
    ...
    let p = checked(
        "#include <string.hc>\nI64 g = 5; U8 *b = MAlloc(32); StrCpy(b, \"hi\"); \"%s %d\\n\", b, g;",
    );
...
fn target_macros_match_each_triple() {
    ...
    for n in ["_WIN32", "_WIN64", "__x86_64__", "__solomon__"] {
    ...
    for n in ["__linux__", "__unix__", "__aarch64__", "__solomon__"] {
...
fn ifdef_target_macro_selects_per_target() {
    ...
    assert_eq!(hcc::irinterp::run_to_string(&win).unwrap(), "1\n");
    assert_eq!(hcc::irinterp::run_to_string(&lin).unwrap(), "2\n");
}

#[test]
fn windows_header_is_inert_off_windows_and_parses_everywhere() {
    let src = "#include <windows.hc>\n\"ok\\n\";";
    let here = Path::new(".");
    for triple in [
        ...
        let p = hcc::parser::parse_with_target(src, here, &[], triple)
            .unwrap_or_else(|e| panic!("parse <windows.hc> failed for {triple}: {e}"));
