...
/// reply (`tests/net/echo.hc`, with `__PORT__` substituted).
fn echo_program(port: u16) -> String {
    include_str!("echo.hc").replace("__PORT__", &port.to_string())
}
...
fn native_arm64_tcp_echo_roundtrip() {
    ...
    let out = std::env::temp_dir().join(format!("hcc-net-{}", std::process::id()));
    Arm64Darwin::new(&out)
        .run(&program)
...
fn native_x86_64_freestanding_tcp_echo() {
    ...
    let port = spawn_echo();
    let out = std::env::temp_dir().join(format!("hcc-x64-net-{}", std::process::id()));
...
fn native_arm64_freestanding_tcp_echo() {
    ...
    let port = spawn_echo();
    let out = std::env::temp_dir().join(format!("hcc-arm-net-{}", std::process::id()));
