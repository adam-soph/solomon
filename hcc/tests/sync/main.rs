...
const PROGRAM: &str = include_str!("atomics.hc");
...
const CONTENDED: &str = include_str!("contended.hc");
...
const CONDVAR: &str = include_str!("condvar.hc");
...
const RWLOCK: &str = include_str!("rwlock.hc");
...
fn native_arm64_sync() {
    ...
    let bin = std::env::temp_dir().join(format!("hcc-sync-{}", std::process::id()));
    Arm64Darwin::new(&bin)
        .run(&compile(PROGRAM))
...
fn native_x86_64_freestanding_sync() {
    ...
    let out = std::env::temp_dir().join(format!("hcc-x64-sync-{}", std::process::id()));
...
fn native_arm64_freestanding_sync() {
    ...
    let out = std::env::temp_dir().join(format!("hcc-arm-sync-{}", std::process::id()));
...
fn native_arm64_contended_mutex() {
    ...
    let bin = std::env::temp_dir().join(format!("hcc-cmu-{}", std::process::id()));
    Arm64Darwin::new(&bin)
...
fn native_arm64_freestanding_contended_mutex() {
    ...
    let out = std::env::temp_dir().join(format!("hcc-arm-cmu-{}", std::process::id()));
...
fn arm64_darwin_stdout(src: &str) -> String {
    let bin = std::env::temp_dir().join(format!("hcc-sd-{}", std::process::id()));
    Arm64Darwin::new(&bin)
        .run(&compile(src))
        .unwrap_or_else(|e| panic!("arm64 build failed: {e}"));
    let output = Command::new(&bin)
...
fn native_arm64_freestanding_condvar_rwlock() {
    ...
    let out = std::env::temp_dir().join(format!("hcc-arm-cr-{}", std::process::id()));
    assert_eq!(
