...
impl<S: TokenStream> Parser<S> {
    ...
    ///
    /// This is the single recursion chokepoint for *type* parsing: generic args, tuple
    /// elements, and anonymous-aggregate fields all recurse back through here, so the depth
    /// guard bounds pathological nesting (e.g. `Vec<Vec<…<I64>…>>`) to a clean error rather
    /// than a stack overflow, the same way `parse_stmt`/`parse_unary` guard their cycles.
    fn parse_base_type(&mut self) -> PResult<Type> {
        self.enter_recursion()?;
        let r = self.parse_base_type_inner();
        self.depth -= 1;
        r
    }

    fn parse_base_type_inner(&mut self) -> PResult<Type> {
...
pub fn parse_in_dir(src: &str, dir: &std::path::Path) -> PResult<Program> {
    parse_core(src, dir, &[], true, crate::host_triple())
}
...
///
/// The predefined target macros default to the **host** triple; use
/// [`parse_with_target`] to compile for another target.
pub fn parse_with(
    src: &str,
    dir: &std::path::Path,
    search: &[std::path::PathBuf],
) -> PResult<Program> {
    parse_core(src, dir, search, true, crate::host_triple())
}

/// Like [`parse_with`], but seeds the predefined target macros (`_WIN32`,
/// `__linux__`, …; see [`crate::target_macros`]) for `triple` instead of the host.
/// The compile/cross-compile and `check`/`ast` CLI paths use this so a program's
/// `#ifdef _WIN32` selects the branch for the target it is being compiled for. An
/// unrecognized `triple` (e.g. `""`) yields just the `__HCC__` marker.
pub fn parse_with_target(
    ...
    parse_core(src, dir, search, true, triple)
}
...
fn parse_core(
    ...
    let known_types = hoist_type_names(src, dir, search, with_builtin, triple)?;
    let mut pp =
        Preprocessor::with_base_dir_and_search(Lexer::new(src), dir.to_path_buf(), search.to_vec());
    // Seed the predefined target macros (`_WIN32`/`__linux__`/…) before any tokens
    // flow, so `#ifdef`-gated platform code resolves for `triple`. This must match
    // the seeding in `hoist_type_names` (below), or a `#ifdef`-gated `class`/`Print`
    // would be hoisted inconsistently.
    pp = pp.with_target(triple);
    if with_builtin {
        // Inject the builtin prelude header `<builtin.hh>` (`NULL`/`TRUE`/`FALSE`, `CTask`,
        // and the sema-injected `argc`/`argv`/`envp`/`Fs`) during preprocessing, read from
        // the stdlib search path. Everything else — the printf family (`<stdio.hh>`), the
        // heap and conversions (`<stdlib.hh>`), the string ops (`<string.hh>`), … — must be
        // `#include`d explicitly, exactly as in C. There is no demand-loading: a program
        // that prints (a `"fmt", …` comma form, a bare string, or a `Print` call) needs
        // `#include <stdio.hh>` itself.
        pp = pp.inject_builtin();
    }
...
fn hoist_type_names(
    ...
    let mut pp =
        Preprocessor::with_base_dir_and_search(Lexer::new(src), dir.to_path_buf(), search.to_vec());
    // Seed the same predefined target macros as `parse_core`, so a `#ifdef`-gated
    // type is hoisted exactly as it will be parsed.
    pp = pp.with_target(triple);
    if with_builtin {
        pp = pp.inject_builtin();
    }
