...
//! populated and [`Layouts`] is available) and produces an [`ir::Program`] in SSA
...
//! [`Load`](crate::ir::Inst::Load)/[`Store`](crate::ir::Inst::Store). A per-function
...
//!
//! This covers the whole implemented language: scalar and memory locals (pointers,
//! arrays, classes, `&`/`*`/`[]`/`.`/`->`), brace/designated initializers, control
//! flow (incl. `switch`), direct/indirect/primitive calls, globals, string literals,
//! by-value aggregate call args/returns, varargs, and exceptions (`try`/`throw`). A
//! handful of deliberately-unsupported constructs still raise a clear "not yet lowered"
//! error.
...
pub fn lower(program: &Program, layouts: &Layouts) -> Result<ir::Program, CodegenError> {
    ...
    // `argc`/`argv`/`envp` are registered just below, each gated on actual use.)
    let mut out_globals: Vec<Global> = Vec::new();
        ...
        match &item.kind {
            StmtKind::Func(func) => {
                if let Some(b) = &func.body {
                    // Collect this function's address-taken names, then drop the ones it
                    // declares locally: `&x` of a function's *own* local `x` cannot alias a
                    // same-named top-level global, so it must not poison that global's
                    // promotability. (The float formatter in `<stdio.hc>` does `&x` on its
                    // local — auto-included into every printing program, it would otherwise
                    // pin every user global named `x` to memory.) Mirrors the shadow-awareness
                    // of `global_used_in_functions`.
                    let mut local_taken = HashSet::new();
                    for s in b {
                        collect_addr_taken(s, &mut local_taken);
                    }
                    for name in local_taken {
                        if !crate::ast::func_declares(func, &name) {
                            addr_taken_prog.insert(name);
                        }
                    }
                }
            }
...
impl<'a> Lowerer<'a> {
    ...
    fn lower_func(&mut self, f: &FuncDef, body: &[Stmt]) -> Result<Func, CodegenError> {
        ...
        if matches!(ret_of(&f.ret, self.layouts), Ret::Agg { .. }) {
        ...
        if !self.cur_terminated {
            // Falling off the end of the top level is normal termination: run the
            // `AtExit` handlers (a no-op for every other function and without
            // `<stdlib.hc>`).
            if self.is_entry {
                self.emit_atexit_run();
            }
    ...
    fn default_ret(&self) -> Term {
        match ret_of(&self.ret_ty, self.layouts) {
            Ret::Void | Ret::Agg { .. } => Term::Ret(None),
            Ret::Scalar(t) if t.is_float() => Term::Ret(Some(Val::ImmF64(0))),
            Ret::Scalar(_) => Term::Ret(Some(Val::ImmInt(0))),
        }
    }
    ...
    fn alloc_var(&mut self, ty: Ty) -> VarId {
    ...
    fn bind_ssa(&mut self, name: &str, ty: Type) -> VarId {
        let irty = scalar_ir_ty(&ty).unwrap_or(Ty::I64);
    ...
    fn slot_addr(&mut self, slot: SlotId, off: u32) -> Val {
        ...
        self.emit(Inst::SlotAddr { dst, slot, off });
        Val::Reg(dst)
    }

    fn global_addr(&mut self, global: GlobalId, off: u32) -> Val {
        let dst = self.fresh_vreg();
        self.emit(Inst::GlobalAddr { dst, global, off });
        Val::Reg(dst)
    }
    ...
    fn expr_ty(&self, e: &Expr) -> Ty {
        e.ty().as_ref().and_then(scalar_ir_ty).unwrap_or(Ty::I64)
    }
...
fn scalar_ir_ty(ty: &Type) -> Option<Ty> {
    Some(match ty {
        Type::I8 => Ty::I8,
        Type::U8 | Type::Bool => Ty::U8,
        Type::I16 => Ty::I16,
        Type::U16 => Ty::U16,
        Type::I32 => Ty::I32,
        Type::U32 => Ty::U32,
        ...
        Type::U64 => Ty::U64,
        Type::F64 => Ty::F64,
        Type::Ptr(_) | Type::FuncPtr { .. } => Ty::Ptr,
        _ => return None,
    })
}

fn ret_of(ty: &Type, layouts: &Layouts) -> Ret {
    match ty {
        Type::U0 => Ret::Void,
        Type::Named(_) | Type::Array(..) => Ret::Agg {
...
impl<'a> crate::lower::Lowerer<'a> {
    ...
    pub(super) fn lower_stmt(&mut self, s: &Stmt) -> Result<(), CodegenError> {
        ...
        match &s.kind {
            ...
            StmtKind::Return(e) => {
                ...
                // A top-level `return` is normal program termination: run the
                // `AtExit` handlers, after the status expression (C `exit` order).
                if self.is_entry {
                    self.emit_atexit_run();
                }
                self.terminate(term);
    ...
    fn lower_try(&mut self, body: &[Stmt], handler: &[Stmt]) -> Result<(), CodegenError> {
        ...
        self.emit(Inst::TryBegin { pad, frame });
    ...
    /// Run the registered `AtExit` handlers: a call to the stdlib's non-public
    /// `__AtExitRun`, injected before a primitive `Exit(code)` call and at `@entry`'s
    /// exits (normal termination, in C terms). A no-op unless `<stdlib.hc>` — which
    /// defines the runner next to the `AtExit` registry — is in the program; `ExitRaw`
    /// (C `_Exit`) and `Abort` skip it by never being spelled `Exit`.
    fn emit_atexit_run(&mut self) {
        if self.defined.contains("__AtExitRun") {
            self.emit(Inst::Call {
                dst: None,
                ret: Ret::Void,
                callee: Callee::Direct("__AtExitRun".to_string()),
                args: vec![],
                sret: None,
        ...
        if self.is_entry && self.scopes.len() == 1 {
            if let Some(&(gid, _)) = self.globals.get(name) {
                ...
                if !self.promotable.contains(name) {
                    // A global with no initializer is already zero in the data region, so
                    // emit no `@entry` store. Besides saving a redundant zeroing, this is
                    // load-bearing for a deferred-impl global (e.g. `<stdlib.hh>`'s
                    // `atexit_n`): its `VarDecl` lands in `@entry` AFTER the user's
                    // top-level code, so a zeroing store here would clobber a value the
                    // user already set through a lib call (`AtExit`).
                    if init.is_some() {
    ...
    /// reached only by fall-through. An all-constant switch lowers to an `Term::Switch`
    ...
    fn lower_switch(
        ...
        let sval = self.coerce(sv, st, Ty::I64);
        ...
        if all_const {
            ...
            for (i, s) in stmts.iter().enumerate() {
                if let StmtKind::Case { lo, hi } = &s.kind {
                    ...
                    let lo_v = self.coerce(lv, lt, Ty::I64);
                    match hi {
                        None => {
                        ...
                        Some(hi) => {
                            ...
                            let hi_v = self.coerce(hv, ht, Ty::I64);
                            // v <= hi ?
                            self.terminate(Term::CondBr {
                                cond: Cond::Cmp {
                                    op: CmpOp::Le,
    ...
    fn emit_string_print(&mut self, s: &str) {
        ...
        self.emit(Inst::StrAddr { dst: addr, str: id });
        self.emit(Inst::Prim {
            dst: None,
            prim: Prim::StdWrite,
...
impl<'a> crate::lower::Lowerer<'a> {
    ...
    pub(super) fn lower_expr(&mut self, e: &Expr) -> Result<(Val, Ty), CodegenError> {
        let pos = e.span.pos;
        match &e.kind {
            ExprKind::Int(v) | ExprKind::Char(v) => Ok((Val::ImmInt(*v), self.expr_ty(e))),
            ExprKind::Float(f) => Ok((Val::ImmF64(f.to_bits()), Ty::F64)),
            ExprKind::Str(s) => {
                ...
                let id = self.strings.intern(s);
                let dst = self.fresh_vreg();
                self.emit(Inst::StrAddr { dst, str: id });
                Ok((Val::Reg(dst), Ty::Ptr))
            }
            ExprKind::Ident(_)
            | ExprKind::Index { .. }
            | ExprKind::Member { .. }
            ...
            ExprKind::Sizeof(arg) => {
                ...
                Ok((Val::ImmInt(sz as i64), Ty::I64))
            }
            ExprKind::Offset { class, path } => {
                ...
                Ok((Val::ImmInt(off as i64), Ty::I64))
            }
            ExprKind::Comma(es) => {
                let mut last = (Val::ImmInt(0), Ty::I64);
                for sub in es {
    ...
    fn lower_lvalue_rvalue(&mut self, e: &Expr) -> Result<(Val, Ty), CodegenError> {
                ...
                Some(info) => {
                    if let Place::Ssa(id) = info.place {
                        if !is_aggregate(&info.ty) {
                            let ty = scalar_ir_ty(&info.ty).unwrap_or(Ty::I64);
                            let cur = self.cur;
    ...
    fn lower_unary(
        ...
        expr: &Expr,
        whole: &Expr,
    ) -> Result<(Val, Ty), CodegenError> {
        match op {
            UnOp::Pos => self.lower_expr(expr),
            UnOp::Neg => {
                let (v, vt) = self.lower_expr(expr)?;
                let ty = if vt.is_float() { Ty::F64 } else { Ty::I64 };
    ...
    fn lower_postfix(&mut self, op: PostOp, expr: &Expr) -> Result<(Val, Ty), CodegenError> {
        let lv = self.lower_lvalue(expr)?;
        let (old, ty) = self.load_lvalue(&lv)?;
    ...
    fn inc_dec(&mut self, old: Val, ty: Ty, inc: bool, lv: &LValue) -> Val {
        if ty == Ty::Ptr {
        ...
        let pty = if ty.is_float() { Ty::F64 } else { Ty::I64 };
        ...
        self.emit(Inst::Bin {
            dst,
            op: if inc { ir::BinOp::Add } else { ir::BinOp::Sub },
            ty: pty,
            signed: true,
            lhs: old,
            rhs: one,
    ...
    fn lower_binary(
        ...
        // Coerce each operand right after lowering it (before lowering the next), so a coercing
        // `Cast` lands next to its operand rather than between the rhs's last instruction and the
        // `Bin`. Evaluation order is unchanged (lhs still lowered before rhs). This keeps a
        // `rhs = b <shift> k` adjacent to the `Bin` that consumes it, which the backend's
        // shifted-register / multiply-add operand fusion requires.
        let (l, lt) = self.lower_expr(lhs)?;
        let l = self.coerce(l, lt, ty);
        let (r, rt) = self.lower_expr(rhs)?;
        let r = self.coerce(r, rt, ty);
    ...
    fn lower_ptr_arith(
        ...
        let mut i = self.coerce(i, it, Ty::I64);
        if op == BinOp::Sub {
    ...
    fn lower_logical(
        ...
        self.terminate(Term::CondBr { cond: c, t, f });
    ...
    fn lower_assign(
        ...
        if op == AssignOp::Assign {
            ...
            return Ok((v, scalar_ir_ty(&ast).unwrap_or(Ty::I64)));
        }

        // Compound assignment. Pointers use scaled add/sub; scalars combine at the
        // promoted width then truncate.
        let target_irty = scalar_ir_ty(&ast).unwrap_or(Ty::I64);
        if target_irty == Ty::Ptr && matches!(op, AssignOp::Add | AssignOp::Sub) {
    ...
    fn lower_indirect_call(
        ...
        let dst = if matches!(ret, Ret::Void | Ret::Agg { .. }) {
            None
        } else {
    ...
    pub(super) fn lower_named_call(
        ...
        if is_prim {
            ...
            let dst = if matches!(ret, Ret::Void) {
                None
            } else {
                Some(self.fresh_vreg())
            };
            // C `exit` runs the registered `atexit` handlers (after the status
            // expression is evaluated). The gate is the spelling: `ExitRaw` (C
            // `_Exit`) maps to the same `Prim::Exit` but skips this.
            if name == "Exit" {
                self.emit_atexit_run();
            }
            self.emit(Inst::Prim {
            ...
            // A failing OS primitive also records its error in the implicit errno
            // (`Fs->err`, read via `<errno.hc>`'s `errno` macro): `if (ret < 0)
            // Fs->err = -ret`, emitted as plain IR so the oracle and every backend
            // agree by construction. Skipped when the prelude's `CTask`/`Fs` aren't
            // in scope (minimal unit-test programs).
            if let Some(d) = dst {
                if prim_sets_errno(prim)
                    && self.globals.contains_key("Fs")
                    && self.layouts.offset_of("CTask", "err").is_some()
                {
                    ...
                    self.seal_block(neg_b);
                    self.switch_to(neg_b);
                    let n = self.fresh_vreg();
                    self.emit(Inst::Un {
                        dst: n,
                        op: ir::UnOp::Neg,
                        ty: Ty::I64,
                    ...
                    self.store_fs_field("err", Val::Reg(n))?;
                    self.terminate(Term::Br(join));
                    self.seal_block(join);
                    self.switch_to(join);
                }
        ...
        if is_varargs {
            ...
            for (k, a) in var_args.iter().enumerate() {
                ...
                let (sty, sval) = if vt.is_float() {
                    ...
                    (Ty::I64, self.coerce(v, vt, Ty::I64))
                };
                self.emit(Inst::Store {
                    ty: sty,
                    addr: at,
    ...
    fn call_result(&self, ret: Ret, dst: Option<Vreg>, sret: Option<Val>) -> (Val, Ty) {
        match ret {
            Ret::Agg { .. } => (sret.unwrap_or(Val::ImmInt(0)), Ty::Ptr),
    ...
    fn alloc_sret(&mut self, ret: Ret) -> Option<Val> {
        if let Ret::Agg { size, align } = ret {
    ...
    fn load_lvalue(&mut self, lv: &LValue) -> Result<(Val, Ty), CodegenError> {
        match lv {
            LValue::Ssa { id, ast } => {
                let ty = scalar_ir_ty(ast).unwrap_or(Ty::I64);
                let cur = self.cur;
    ...
    pub(super) fn coerce(&mut self, val: Val, from: Ty, to: Ty) -> Val {
        if from == to {
            return val;
        }
        let dst = self.fresh_vreg();
...
/// The fail-able OS primitives whose negative `-errno` return is also recorded in the
/// implicit errno (`Fs->err`) by an injected `if (ret < 0) Fs->err = -ret` — see
/// [`Lowerer::lower_named_call`]. C sets errno only on failure and leaves it alone on
/// success; the injection matches that.
fn prim_sets_errno(prim: Prim) -> bool {
    matches!(
        prim,
        Prim::Open
            | Prim::Read
...
fn promoted(lhs: &Expr, rhs: &Expr) -> Ty {
...
fn arith_op(op: BinOp) -> Option<ir::BinOp> {
    Some(match op {
