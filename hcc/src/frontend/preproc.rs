...
/// A `.hc` implementation queued by a `.hh` header (filename auto-pairing) or an
/// explicit `#impl` directive, to be streamed **after** the primary source and all its
/// header includes are exhausted. Deferring the bodies to a flat end-batch (rather than
/// pasting them at the include site) is what keeps the header layer cycle-free: a header
/// only ever needs another header's *declarations*, so the impls can reference each other
/// freely once they all land at the end. Resolved eagerly (so a missing impl errors at the
/// include site), pushed lazily as an [`IncludeFrame`] when the drain reaches it.
struct Deferred {
    path: PathBuf,
    dir: PathBuf,
    contents: String,
    display: String,
    info: FileInfo,
    /// The include-site position, for a cycle/depth error if the impl can't be pushed.
    pos: Pos,
}
...
pub struct Preprocessor<S: TokenStream> {
    ...
    exe_count: usize,
    /// Implementations (`.hc`) queued by `.hh` headers / `#impl`, streamed after the
    /// primary source and its includes are exhausted (FIFO). See [`Deferred`].
    deferred: VecDeque<Deferred>,
    /// Canonical keys of impls already queued, so each is streamed at most once across
    /// the whole parse (the file's own `#ifndef` guard is a second line of defence).
    deferred_seen: HashSet<String>,
}
...
impl<S: TokenStream> Preprocessor<S> {
    ...
    /// Implicitly include the builtin prelude header `<builtin.hh>` ahead of the base
    /// source, so its `NULL`/`TRUE`/`FALSE`, `CTask`, and the sema-injected
    /// `argc`/`argv`/`envp`/`Fs` are in scope without an explicit `#include`. The
    /// builtin **contents are injected here, during preprocessing**, read from the
    /// standard-library search path like any angle include — it is no longer embedded
    /// in the compiler, and there is no separate "prelude" string. Pushed as the first
    /// frame on the include stack, so its tokens come first and its line numbers don't
    /// shift the user program's. It is a **public** root file for privacy purposes, so
    /// its builtins are callable from user code.
    ///
    /// A no-op if `builtin.hh` cannot be found on the search path (e.g. a raw front end
    /// with no standard library available) — the front end then simply lacks the
    /// builtins, surfacing as ordinary "undeclared" errors rather than a panic.
    pub fn inject_builtin(mut self) -> Self {
        let Some(contents) = self
            .search
            .iter()
            .cloned()
            .chain(crate::stdlib_dirs())
            .find_map(|d| std::fs::read_to_string(d.join("builtin.hh")).ok())
        else {
            return self;
        };
    ...
    ///
    /// Unlike [`inject_builtin`](Self::inject_builtin), these go straight into the macro
        ...
        loop {
            ...
            match &t.kind {
                TokenKind::Eof => {
                    ...
                    // The primary source and all its header includes are exhausted; now
                    // stream the deferred `.hc` implementations queued by `.hh` headers
                    // (FIFO). Each is pushed as an ordinary frame, so its own Eof pops it
                    // here and the loop returns to drain the next — and an impl's `.hh`
                    // includes queue *their* impls, so the batch drains transitively.
                    if let Some(d) = self.deferred.pop_front() {
                        self.push_frame(d.path, d.dir, d.contents, &d.display, d.pos, d.info)?;
                        continue;
                    }
    ...
    fn do_exe(&mut self, pos: Pos) -> LResult<()> {
        ...
        let out = crate::oracle::run_to_string(&program)
    ...
    fn do_include(&mut self, toks: &[Token]) -> LResult<()> {
        ...
        match toks.get(1).map(|t| &t.kind) {
            Some(TokenKind::Str(p)) => {
                ...
                // A `.hh` header auto-pairs with its sibling `.hc` implementation,
                // streamed at the end (deferred). No-op if there is no sibling impl.
                self.queue_paired_impl(&path_str, Some(&cur_dir), pos);
                Ok(())
            }
            Some(TokenKind::Lt) => {
                ...
                // Resolve against the explicit search path (`-I` and caller-provided
                // dirs), then the default standard-library directories (`HCC_STDLIB`,
                // `$HCC_ROOT/lib`, …; see [`crate::stdlib_dirs`]). The standard library is
                // read from disk — it is no longer embedded in the compiler.
                ...
                {
                    let info = file_info_for_disk(&canon);
                    self.open_include(canon, &display, pos, info)?;
                    self.queue_paired_impl(&path_str, None, pos);
                    return Ok(());
                }
                Err(self.err(
                    pos,
                    format!(
                        "cannot find #include <{path_str}> in the search path \
                         (set HCC_ROOT or HCC_STDLIB, or pass -I)"
                    ),
                ))
            }
    ...
    /// Resolve an include `name` to a streamable source, the same way `do_include`
    /// does — a quote include against `quote_dir`, an angle include against the search
    /// path then the embedded stdlib — but returning the resolved pieces instead of
    /// pushing a frame. Used to queue a deferred implementation. Returns `None` when the
    /// name doesn't resolve (e.g. a header with no sibling `.hc` — a header-only module).
    fn resolve_source(&self, name: &str, quote_dir: Option<&Path>) -> Option<Deferred> {
        if let Some(dir) = quote_dir {
            let canon = dir.join(name).canonicalize().ok()?;
        ...
        // Angle: explicit search path first, then the default standard-library dirs.
        let canon = self
            .search
            .iter()
            .cloned()
            .chain(crate::stdlib_dirs())
            .find_map(|d| d.join(name).canonicalize().ok())?;
        let contents = std::fs::read_to_string(&canon).ok()?;
        let frame_dir = canon
            .parent()
            .map(|p| p.to_path_buf())
    ...
    /// Queue a deferred entry, deduped by its canonical key, with `pos` stamped for
    /// error reporting if it can't later be pushed.
    fn queue(&mut self, mut d: Deferred, pos: Pos) {
    ...
    /// Filename auto-pairing: a `#include` of `name` ending in `.hh` queues its sibling
    /// `.hc` implementation (same basename, resolved the same way) to be streamed at the
    /// end. A no-op for any other extension, or when no sibling impl exists (a header-only
    /// module like `<limits.hh>`).
    fn queue_paired_impl(&mut self, name: &str, quote_dir: Option<&Path>, pos: Pos) {
        let Some(stem) = name.strip_suffix(".hh") else {
            return;
        };
        let impl_name = format!("{stem}.hc");
        if let Some(d) = self.resolve_source(&impl_name, quote_dir) {
            self.queue(d, pos);
        }
    }

    /// `#impl <name>` / `#impl "name"` — queue an implementation explicitly, for a header
    /// whose impl isn't found by filename auto-pairing (a differently-named or split impl).
    /// Unlike auto-pairing, an unresolvable name here is an error: it was named on purpose.
    fn do_impl(&mut self, toks: &[Token]) -> LResult<()> {
        let pos = toks[0].span.pos;
        let (name, quote_dir) = match toks.get(1).map(|t| &t.kind) {
            Some(TokenKind::Str(p)) => {
                let dir = self
                    .includes
                    .last()
            ...
            Some(TokenKind::Lt) => {
                let p = angle_path(&toks[1..])
                    .ok_or_else(|| self.err(pos, "malformed #impl <...> path"))?;
                (p, None)
            }
            _ => return Err(self.err(pos, "#impl expects \"file\" or <name>")),
        };
        match self.resolve_source(&name, quote_dir.as_deref()) {
            Some(d) => {
                self.queue(d, pos);
                Ok(())
            }
            None => Err(self.err(pos, format!("cannot find #impl <{name}>"))),
        }
    }
    ...
    fn push_frame(
        ...
        // Streaming an implementation (`.hc`) inline — a direct `#include <foo.hc>`, or
        // the deferred drain pushing one — marks it already-pulled, so its own
        // `#include <foo.hh>` won't auto-pair the same impl a second time (which would
        // double-define every body). The key matches the one `queue` uses.
        let key = path.to_string_lossy();
        if key.ends_with(".hc") || key.ends_with(".hc>") {
            self.deferred_seen.insert(key.into_owned());
        }
