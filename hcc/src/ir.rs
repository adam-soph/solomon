//! A low-level, SSA-form intermediate representation for hcc.
//!
//! Historically hcc had **no IR**: the [oracle](crate::oracle) and both
...
//! It is "near machine code": memory is explicit ([`Inst::Load`]/[`Inst::Store`]
//! with a width), addressing is explicit ([`Inst::SlotAddr`]/[`Inst::PtrAdd`]),
...
//! [`Ty`]; they are referenced by their address.
...
pub type Vreg = u32;
/// A basic-block id, an index into [`Func::blocks`].
pub type BlockId = u32;
/// A frame-slot (`alloca`) id, an index into [`Func::slots`].
pub type SlotId = u32;
/// An interned string-literal id, an index into [`Program::strings`].
pub type StrId = u32;
/// A global id, an index into [`Program::globals`].
...
pub struct Program {
    ...
    /// Interned, NUL-terminated string-literal bytes. [`Inst::StrAddr`] targets
...
pub enum Inst {
    ...
    Load { dst: Vreg, ty: Ty, addr: Val },
    /// `*(ty *)addr = val`, truncating `val` to `ty`.
    Store { ty: Ty, addr: Val, val: Val },
...
/// Arithmetic / bitwise / shift binary operators. Comparisons are [`Inst::Cmp`] /
...
pub enum UnOp {
    Neg,
    BitNot,
    /// 64-bit population count (set-bit count). Produced only by loop-idiom recognition
    /// (`crate::backend::idiom`); the arm64 backend lowers it to `cnt`, the x86 backend expands
    /// it to a SWAR sequence. The interpreter never sees it (idiom recognition is backend-only),
    /// but implements it for completeness.
    Popcount,
}
...
/// is pure HolyC, lowered as ordinary [`Inst::Call`]s that bottom out at
/// [`Prim::StdWrite`].
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Prim {
    StdWrite,
    ...
    Mkdir,
    Exit,
    /// Run a shell command (`system`): `/bin/sh -c cmd`, returning the child's exit
    /// code (0–255), or a negative value when the spawn/wait fails or the child dies
    /// abnormally.
    System,
    Getpid,
    Getppid,
    ...
    Chdir,
    Getcwd,
    /// The calling thread's OS thread id (`gettid`): a small per-thread integer,
    /// stable for the thread's lifetime — the key pure-HolyC TLS uses.
    Gettid,
    Thread,
    Join,
    /// Yield the processor (`sched_yield`); always returns 0.
    ThreadYield,
    /// Terminate the calling thread with a result value (`thrd_exit`): `Join` sees
    /// `ret` exactly as if the thread function had returned it. From the main flow
    /// (outside any thread) it terminates the program with `ret` as the exit status.
    ThreadExit,
    /// Mark a thread as never-joined (`pthread_detach`). Freestanding threads are a
    /// no-op (their stacks are never reclaimed either way); returns 0.
    ThreadDetach,
    AtomicLoad,
    AtomicStore,
    AtomicAdd,
    ...
    FutexWait,
    FutexWake,
    /// `FutexWait` with a caller-supplied timeout in nanoseconds: sleep while the
    /// 32-bit word at `addr` equals `expected`, at most `ns`. Returns 0 when woken,
    /// or a negative value on timeout/value-mismatch — the exact code is
    /// target-flavoured, so callers should re-check their predicate (or a deadline),
    /// not the code.
    FutexWaitNs,
...
impl Prim {
    ...
    pub fn from_name(name: &str) -> Option<Prim> {
        ...
        Some(match name {
            ...
            "Mkdir" => Prim::Mkdir,
            "Exit" => Prim::Exit,
            // C `_Exit`: the same primitive — only lowering treats the two names
            // differently (no `AtExit`-handler run before `ExitRaw`).
            "ExitRaw" => Prim::Exit,
            "System" => Prim::System,
            "Getpid" => Prim::Getpid,
            ...
            "ThreadDetach" => Prim::ThreadDetach,
            "ThreadExit" => Prim::ThreadExit,
            "Gettid" => Prim::Gettid,
...
impl Term {
    ...
    pub fn successors(&self) -> Vec<BlockId> {
        match self {
            ...
            Term::Ret(_) | Term::Throw(_) | Term::Rethrow | Term::Unreachable => vec![],
        }
    }

    /// Visit each `Val` operand this terminator reads (branch targets are *not* operands;
    /// see [`successors`](Term::successors)). The single source of truth shared by liveness
    ...
    pub fn for_each_use(&self, mut f: impl FnMut(Val)) {
        match self {
            ...
            Term::Ret(Some(val)) | Term::Throw(val) => f(*val),
            Term::Br(_) | Term::Ret(None) | Term::Rethrow | Term::Unreachable => {}
        }
    }

    /// The mutable mirror of [`for_each_use`](Term::for_each_use), for operand rewrites (use
...
impl Inst {
    ...
    pub fn def(&self) -> Option<Vreg> {
        match *self {
            ...
            Inst::Call { dst, .. } | Inst::Prim { dst, .. } => dst,
            Inst::Store { .. }
            | Inst::MemCpy { .. }
            | Inst::MemZero { .. }
            | Inst::TryBegin { .. }
    ...
    /// A mutable reference to the [`Vreg`] this instruction defines, or `None` for an
    /// effect-only instruction. The mirror of [`def`](Inst::def), for vreg renumbering (e.g.
    /// the inliner offsetting a cloned callee's registers).
    pub fn def_mut(&mut self) -> Option<&mut Vreg> {
        match self {
            ...
            | Inst::PtrAdd { dst, .. }
            | Inst::Load { dst, .. } => Some(dst),
            Inst::Call { dst, .. } | Inst::Prim { dst, .. } => dst.as_mut(),
            Inst::Store { .. }
            | Inst::MemCpy { .. }
            | Inst::MemZero { .. }
            | Inst::TryBegin { .. }
    ...
    pub fn for_each_use(&self, mut f: impl FnMut(Val)) {
        match self {
            ...
            Inst::Un { src, .. } | Inst::Cast { src, .. } | Inst::Mov { src, .. } => f(*src),
    ...
    /// The mutable mirror of [`for_each_use`](Inst::for_each_use), for operand rewrites (use
...
/// Structural sanity checks over an [`Program`]: every block/slot/string/global
...
fn verify_func(f: &Func, p: &Program, errs: &mut Vec<String>) {
...
/// range. Operand `Val` uses are checked separately via [`Inst::for_each_use`].
fn inst_check_refs(inst: &Inst, f: &Func, p: &Program, errs: &mut Vec<String>) {
