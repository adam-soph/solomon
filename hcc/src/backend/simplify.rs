...
//!   the *same* functions the oracle runs ([`crate::oracle::bin`] / [`crate::oracle::cmp`]),
...
fn eliminate_trivial_phis(f: &mut Func) {
    loop {
...
fn replace_all_uses(f: &mut Func, from: Vreg, to: Val) {
...
/// unchanged.
fn simplify_inst(inst: Inst, out: &mut Vec<Inst>) {
    match inst {
        Inst::Bin {
            dst,
            op,
...
fn try_int_rewrite(
    ...
    let mov = |out: &mut Vec<Inst>, src: Val| out.push(Inst::Mov { dst, ty, src });
    let shl = |out: &mut Vec<Inst>, x: Val, k: i64| {
        out.push(Inst::Bin {
            dst,
            op: BinOp::Shl,
            ty,
