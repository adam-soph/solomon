...
pub(crate) fn run(mut p: Program) -> Program {
...
fn hoist_loop(f: &mut Func, target: BlockId, body: &HashSet<BlockId>, mem: &MemInfo) {
        ...
        for &b in &body_blocks {
            ...
            for inst in blk.insts.drain(..) {
                ...
                let ok = all_uses_invariant(&inst, invariant)
                    && match &inst {
                        Inst::Load { addr, .. } => load_hoistable(*addr, mem, &eff),
...
fn def_blocks(f: &Func) -> Vec<BlockId> {
...
fn hoistable(i: &Inst) -> bool {
    match i {
        ...
        Inst::Bin { op, ty, .. } => !(matches!(op, BinOp::Div | BinOp::Mod) && !ty.is_float()),
        Inst::Un { .. }
        | Inst::Cast { .. }
        | Inst::Cmp { .. }
        | Inst::PtrAdd { .. }
...
fn all_uses_invariant(i: &Inst, invariant: impl Fn(Vreg) -> bool) -> bool {
    let mut ok = true;
...
impl MemInfo {
    fn new(f: &Func) -> MemInfo {
        ...
        for b in &f.blocks {
            for i in &b.insts {
                match i {
                    Inst::Load { .. } | Inst::MemZero { .. } | Inst::MemCpy { .. } => {}
                    Inst::Store { val, .. } => {
...
impl LoopEffects {
    fn new(f: &Func, body: &[BlockId], mem: &MemInfo) -> LoopEffects {
        let mut has_call = false;
        let mut written: HashSet<MemBase> = HashSet::new();
        for &b in body {
            for i in &f.blocks[b as usize].insts {
                match i {
                    Inst::Call { .. } | Inst::Prim { .. } => has_call = true,
                    Inst::Store { addr, .. } => {
                        if let Some(mb) = mem.addr_base(*addr) {
                            written.insert(mb);
                        }
                    }
                    Inst::MemZero { dst, .. } | Inst::MemCpy { dst, .. } => {
