...
pub fn allocate(f: &Func, regs: &RegSet, program_has_exceptions: bool) -> Allocation {
    ...
    for (bi, b) in f.blocks.iter().enumerate() {
        ...
        for inst in b.insts.iter().rev() {
            if matches!(inst, Inst::Call { .. } | Inst::Prim { .. }) {
    ...
    // Candidates, processed in start order (the linear scan's expiry needs that), with
    // eviction by weight when registers run out. Two tiers:
    //   * refs ≥ 2 — the original hot-vreg tier: prefers caller-saved, falls back to
    //     callee-saved (a prologue save), and may evict a strictly-cheaper active holder.
    //   * refs == 1 *and* non-crossing — single-use temps (most of every expression chain;
    //     each otherwise round-trips through a frame slot). Constrained below to a free
    //     caller-saved register only: no callee fallback (a prologue save to dodge one
    //     ldr/str pair is a bad trade) and no eviction (they only ever grab idle registers,
    //     and may themselves be evicted by a hotter vreg — both safe, the temp just respills).
    //     A single-use *crossing* temp is excluded outright (it would force a callee-saved
    //     register, i.e. a prologue save, for a single avoided spill).
    let mut cands: Vec<usize> = (0..n)
        .filter(|&v| start[v] != u32::MAX && (refs[v] >= 2 || (refs[v] == 1 && !crossing[v])))
        .collect();
    ...
    for &v in &cands {
        ...
        let pick = if cr {
            ...
            // Single-use non-crossing temp: a free caller-saved register or nothing.
            caller.pop().map(|r| (r, true))
        } else {
            caller
                .pop()
                .map(|r| (r, true))
        ...
        let pick = pick.or_else(|| {
            ...
            // Single-use temps never evict (they take only an idle register, above).
            if single_use {
                return None;
            }
            let victim = active
