...
fn apply(f: &mut Func, p: Plan) {
    ...
    f.blocks[p.preheader as usize].insts.push(Inst::PtrAdd {
        dst: q0,
        base: p.base,
        index: p.k0,
        stride: p.stride,
    });
    f.blocks[p.latch as usize].insts.push(Inst::PtrAdd {
        dst: qnext,
        base: Val::Reg(q),
...
fn iv_step(f: &Func, body: &HashSet<BlockId>, knext: Vreg, k: Vreg) -> Option<i64> {
    for &b in body {
        for inst in &f.blocks[b as usize].insts {
            if let Inst::Bin {
                dst,
