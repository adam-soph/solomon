...
use crate::backend::x86_64::{load_opcode, store_opcode};
...
impl Asm {
    ...
    // Register-offset (SIB) memory forms `[rcx + rsi*(1<<scale_log2)]`, the addressing-mode
    // fusion (T3) of an array element's address into the load/store. Base rcx and index rsi
    // are fixed *low* registers, so REX.X/REX.B are never set and the `load_opcode`/
    // `store_opcode` prefixes apply unchanged; `scale_log2` is 0..=3 for stride 1/2/4/8.
    // ModRM `0x04` = mod 00, reg rax (000), rm 100 (SIB follows); SIB = scale|rsi(110)|rcx(001).
    /// `rax = [rcx + rsi*scale]`, sign/zero-extended per `signed` — the SIB `load_through`.
    pub(super) fn load_sib(&mut self, size: i32, signed: bool, scale_log2: u8) {
        self.emit(load_opcode(size, signed));
        self.emit(&[0x04, (scale_log2 << 6) | (6 << 3) | 1]);
    }
    /// `[rcx + rsi*scale] = rax` (low `size` bytes) — the SIB `store_through`.
    pub(super) fn store_sib(&mut self, size: i32, scale_log2: u8) {
        self.emit(store_opcode(size));
        self.emit(&[0x04, (scale_log2 << 6) | (6 << 3) | 1]);
    }
    /// `xmm0 = [rcx + rsi*scale]` (an F64) — the SIB `movsd_load_at`.
    pub(super) fn movsd_load_sib(&mut self, scale_log2: u8) {
        self.emit(&[0xF2, 0x0F, 0x10, 0x04, (scale_log2 << 6) | (6 << 3) | 1]);
    }
    /// `[rcx + rsi*scale] = xmm0` (an F64) — the SIB `movsd_store_at`.
    pub(super) fn movsd_store_sib(&mut self, scale_log2: u8) {
        self.emit(&[0xF2, 0x0F, 0x11, 0x04, (scale_log2 << 6) | (6 << 3) | 1]);
    }
    /// `imul rax, rax, imm32`
    ...
    /// NaN-aware ordered float equality after a `ucomisd`: `a == b` is true only when the
    /// operands are ordered *and* equal, i.e. `ZF=1 && PF=0` (`ucomisd` sets `PF=1` when an
    /// operand is NaN). Result 0/1 in rax; clobbers cl.
    pub(super) fn float_eq_movzx(&mut self) {
        self.emit(&[0x0F, 0x94, 0xC0]); // sete  al   (ZF)
        self.emit(&[0x0F, 0x9B, 0xC1]); // setnp cl   (PF == 0)
        self.emit(&[0x20, 0xC8]); // and al, cl
        self.emit(&[0x0F, 0xB6, 0xC0]); // movzx eax, al
    }
    /// NaN-aware float inequality after a `ucomisd`: `a != b` is true when the operands are
    /// unordered *or* not equal, i.e. `ZF=0 || PF=1`. Result 0/1 in rax; clobbers cl.
    pub(super) fn float_ne_movzx(&mut self) {
        self.emit(&[0x0F, 0x95, 0xC0]); // setne al   (ZF == 0)
        self.emit(&[0x0F, 0x9A, 0xC1]); // setp  cl   (PF)
        self.emit(&[0x08, 0xC8]); // or al, cl
        self.emit(&[0x0F, 0xB6, 0xC0]); // movzx eax, al
    }
    ...
    /// `mov qword [rsp + disp8], reg` (64-bit), REX.R-capable so r8–r15 work.
    pub(super) fn store_rsp_reg(&mut self, disp: i8, reg: u8) {
        let rex = 0x48 | if reg >= 8 { 0x04 } else { 0 };
        self.emit(&[rex, 0x89, 0x44 | ((reg & 7) << 3), 0x24, disp as u8]);
    }
    /// `sub rsp, imm32`.
    pub(super) fn sub_rsp(&mut self, imm: i32) {
        self.alu_ri(5, 4, imm); // group-1 ext 5 = sub; reg 4 = rsp
    }
    ...
    pub(super) fn sub_ri(&mut self, rm: u8, imm: i32) {
        self.alu_ri(5, rm, imm);
    }
    pub(super) fn xor_ri(&mut self, rm: u8, imm: i32) {
        self.alu_ri(6, rm, imm);
    }
    /// A shift by an immediate count (`C1 /ext ib`, 64-bit): ext 4 shl, 5 shr, 7 sar.
    pub(super) fn shift_ri(&mut self, ext: u8, rm: u8, imm: u8) {
        self.emit(&[rex_b1(rm), 0xC1, 0xC0 | (ext << 3) | (rm & 7), imm]);
    }
    pub(super) fn shl_ri(&mut self, rm: u8, imm: u8) {
        self.shift_ri(4, rm, imm);
    }
    pub(super) fn shr_ri(&mut self, rm: u8, imm: u8) {
        self.shift_ri(5, rm, imm);
    }
    pub(super) fn sar_ri(&mut self, rm: u8, imm: u8) {
        self.shift_ri(7, rm, imm);
    }
