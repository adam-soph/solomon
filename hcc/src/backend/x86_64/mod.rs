...
trait OsTarget {
    ...
    /// Reserve (or look up) an import slot for the named external function, for a
    /// `Prim::WinCall` lowered to a direct call. Returns the IAT slot index, or `None`
    /// on a target that has no dynamic imports — the non-Windows targets, where a
    /// `Prim::WinCall` is rejected. Windows returns `Some(extern_idx(name))`.
    fn extern_slot(&mut self, _name: &'static str) -> Option<usize> {
        None
    }
