...
//!    a type-checked [`Program`](crate::ast::Program) to a binary. [`CodegenError`] is the error these (and the IR
//!    [oracle](crate::oracle)) raise.
//!
...
//!    work the two backends share and the IR [oracle](crate::oracle) skips entirely
...
pub trait Codegen {
    ...
    fn run(&mut self, program: &crate::ast::Program) -> Result<(), CodegenError>;
}
...
/// Lower a type-checked AST `program` to the phi-free, out-of-SSA [`Program`] that both
...
/// same in `oracle::run_to_string_with_input`, so all three consumers reject the same
...
pub fn lower_to_machine_ir(program: &crate::ast::Program) -> Result<Program, CodegenError> {
    ...
    // Loop-idiom recognition (Kernighan popcount → constant-time bit twiddling), before CSE so
    // the emitted sequence is value-numbered and the `c0 + popcount` folds. Backend path only.
    let ir = idiom::run(ir);
    debug_assert!(
        crate::ir::verify(&ir).is_empty(),
        "idiom recognition produced invalid IR: {:?}",
        crate::ir::verify(&ir)
    );
    // Inline small non-recursive callees, before CSE/LICM so the spliced-in body is exposed to
    // them and the callee's loop-carried scalars promote in the caller. Backend path only.
    let ir = inline::run(ir);
    debug_assert!(
        crate::ir::verify(&ir).is_empty(),
        "inlining produced invalid IR: {:?}",
        crate::ir::verify(&ir)
    );
    // Common-subexpression elimination + no-op cast/copy propagation + block-local
    // redundant-load elimination (backend path only). Runs before LICM so a deduplicated
    // computation is hoisted once and merged values reach the ≥2 refs the register promoter
    // wants. Semantics-preserving — validated by the goldens.
    let ir = cse::run(ir);
    debug_assert!(
        crate::ir::verify(&ir).is_empty(),
    ...
    // Dead-code elimination: sweep pure computations (and dead phis) left unused by the
    // preceding passes — superseded IVSR address recurrences, CSE-orphaned duplicates, and
    // no-longer-referenced casts. Last, so it cleans up after everything. Backend path only.
    let ir = dce::run(ir);
    debug_assert!(
        crate::ir::verify(&ir).is_empty(),
...
pub fn reachable_functions<'a>(
    ir: &'a Program,
    backend: &str,
) -> Result<Vec<&'a Func>, CodegenError> {
    let by_name: HashMap<&str, &Func> = ir.funcs.iter().map(|f| (f.name.as_str(), f)).collect();
    let mut reachable: Vec<&Func> = Vec::new();
        ...
        for b in &f.blocks {
            for inst in &b.insts {
                match inst {
                    ...
                    Inst::FuncAddr { func, .. } => queue.push(func),
                    _ => {}
                }
            }
        }
...
pub fn heap_prims_used(funcs: &[&Func]) -> HashSet<&'static str> {
    let mut used = HashSet::new();
    for f in funcs {
        for b in &f.blocks {
            for inst in &b.insts {
                if let Inst::Prim { prim, .. } = inst {
                    match prim {
...
pub fn program_has_exceptions(ir: &Program) -> bool {
    ir.funcs.iter().any(|f| {
        f.blocks.iter().any(|b| {
            matches!(b.term, Term::Throw(_) | Term::Rethrow)
                || b.insts
                    .iter()
                    .any(|i| matches!(i, Inst::TryBegin { .. } | Inst::TryEnd))
        })
    })
}
...
pub fn func_uses_fs(f: &Func, fs_gid: GlobalId) -> bool {
    f.blocks.iter().any(|b| {
        matches!(b.term, Term::Throw(_) | Term::Rethrow)
            || b.insts.iter().any(|i| match i {
                Inst::TryBegin { .. } | Inst::TryEnd => true,
                Inst::GlobalAddr { global, .. } => *global == fs_gid,
                _ => false,
            })
    })
}
...
pub fn prog_uses_fs(reachable: &[&Func], fs: Option<GlobalId>) -> bool {
...
pub fn implicit_globals(ir: &Program) -> ImplicitGlobals {
    let gid = |name: &str| {
        ir.globals
            .iter()
...
pub fn ctask_layout(ir: &Program) -> Result<CTaskLayout, CodegenError> {
    let field = |name: &str| {
...
pub fn emit_blocks<B: Backend>(b: &mut B, f: &Func) -> Result<(), CodegenError> {
...
pub fn destruct_ssa(f: &Func) -> Func {
    let mut blocks: Vec<Block> = f.blocks.clone();
    ...
    for b_id in 0..blocks.len() {
        ...
        for a in preds[b_id].clone() {
            ...
            let copies: Vec<(Vreg, Ty, Val)> = phis
                .iter()
                .map(|phi| {
                    let val = phi
                        .args
...
pub fn destruct_program(p: &Program) -> Program {
    Program {
...
fn sequence_copies(copies: &[(Vreg, Ty, Val)], next_vreg: &mut u32) -> Vec<Inst> {
    // Drop no-op self-copies (`dst = dst`); they constrain nothing and need no move.
    let copies: Vec<(Vreg, Ty, Val)> = copies
        .iter()
...
fn redirect_term(term: &mut Term, from: BlockId, to: BlockId) {
