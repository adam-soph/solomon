...
//! Darwin backend, via the [`asm`](crate::backend::arm64::asm) module and the parent's `compile`
...
use crate::backend::arm64::ArmTarget;
...
impl Arm64Linux {
    ...
    pub fn object(&self, program: &Program) -> Result<Vec<u8>, CodegenError> {
        ...
        crate::backend::arm64::isel::compile_ir(&ir, &Linux)
    }
}
...
impl Codegen for Arm64Linux {
    ...
    fn run(&mut self, program: &Program) -> Result<(), CodegenError> {
        crate::backend::arm64::build(program, &self.out_path, &Linux)
    }
}
