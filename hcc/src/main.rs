//! The `hcc` CLI: the HolyC compiler.
//!
//! Reads HolyC source from a file, or stdin, and by default compiles a native
//! binary for the host's architecture and OS. The `check`/`ast`/`tokens`
//! subcommands are front-end-only. (The IR interpreter — the conformance oracle —
//! is a library API, `hcc::oracle`, not exposed on the CLI.)
//!
//! Usage:
...
fn main() -> ExitCode {
    ...
    // seed: `--target` if given, else the host, else `""` (the `__HCC__`-only
