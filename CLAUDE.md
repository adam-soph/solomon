...
named for its target — `arm64_darwin` (`aarch64-apple-darwin`, Mach-O via `cc`)
and `x86_64_linux` (`x86_64-unknown-linux`, a freestanding static ELF). A codegen
backend is an (arch, OS) pair, not just a CPU.
...
- **`backend/x86_64_linux.rs`** — a second native target (Linux/x86-64, *integers
  ...
  *after* the code, RIP-relative addressed; `Asm::finish` resolves rel32 branch
  ...
  `Abs`/`Sign`, `RandU64` (the shared splitmix64 over a zero-seeded BSS word, so
  ...
  Only `MStrPrint`/`F64ToStr` aren't done; each errors at build time. (**All 18
  examples now compile and run natively**, matching the interpreter.) Tests in
...
`llabs`); memory
`MemCpy`/`MemMove`/`MemSet`/`MemCmp`/`MemFind`/`MemSearch`/`MAlloc`/`Free`; char `ToUpper`/`ToLower`; and the
exactly-reproducible float ops `Sqrt`/`Floor`/`Ceil`/`Round`/`Fabs`/`Sign`; plus
`RandU64`. (The **transcendentals** — `Sin`/`Cos`/`Pow`/`Exp`/`Ln`/… — are
