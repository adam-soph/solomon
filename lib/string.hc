// string.hc — the solomon standard string and number-conversion library.
//
// Pure HolyC over byte pointers (`U8 *`). These were once compiler builtins; they
// are ordinary library code now, built on indexing and pointer arithmetic, so they
// compute identically on the interpreter and every native backend. Byte values are
// `U8` (unsigned), so the `<`/`>` comparisons are unsigned — matching libc's
// `strcmp` family. Include with `#include <string.hc>`.
