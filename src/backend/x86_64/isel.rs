...
impl FnEmit<'_> {
    fn emit(&mut self, f: &IrFunc) -> Result<(), CodegenError> {
        // Callee-saved pool rbx/r12–r14 + the one free caller-saved GPR r10 (used only for
        // ranges that don't cross a call). No float pool yet — `load_float`/`store_float`
        // don't consult the allocation until a later phase adds xmm promotion. Safe on
        // Windows now that the prologue stack-probes large frames (see `Asm::prologue_probe`).
        let regset = crate::backend::RegSet {
            callee_int: PROMOTE_INT.to_vec(),
            caller_int: vec![R10 as u32],
            callee_flt: Vec::new(),
            caller_flt: Vec::new(),
        };
    ...
    fn try_imm_binop(&mut self, op: IrBinOp, signed: bool, imm: i64) -> bool {
        ...
        let i = imm as i32;
        match op {
            IrBinOp::Add if fits32 => self.asm.add_ri(TMP0, i),
            IrBinOp::Sub if fits32 => self.asm.sub_ri(TMP0, i),
            IrBinOp::BitAnd if fits32 => self.asm.and_ri(TMP0, i),
            IrBinOp::BitOr if fits32 => self.asm.or_ri(TMP0, i),
            IrBinOp::BitXor if fits32 => self.asm.xor_ri(TMP0, i),
            IrBinOp::Shl if (0..64).contains(&imm) => self.asm.shl_ri(TMP0, imm as u8),
            IrBinOp::Shr if !signed && (0..64).contains(&imm) => self.asm.shr_ri(TMP0, imm as u8),
            IrBinOp::Shr if signed && (0..64).contains(&imm) => self.asm.sar_ri(TMP0, imm as u8),
            _ => return false,
        }
        true
    }
