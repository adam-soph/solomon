...
use crate::backend::x86_64::{
    Asm, FileOp, OsTarget, R8, R9, R10, R11, R15, RAX, RCX, RDI, RDX, RSI, RSP,
};
...
impl Codegen for X64Windows {
    ...
    fn run(&mut self, program: &Program) -> Result<(), CodegenError> {
        ...
        let pe = crate::backend::x86_64::isel::compile_ir(&ir, Box::new(WindowsTarget::new()))?;
...
struct WindowsTarget {
    /// Imported functions, as `(dll, name)` pairs, in import-table (and IAT) order.
    /// The index of each is the slot `call_extern` records for the call site. Most are
    /// `kernel32.dll`; a `Prim::WinCall` may name another DLL (e.g. `advapi32.dll` for
    /// the registry). The import directory groups them by DLL.
...
impl WindowsTarget {
    ...
    /// The import slot for a `kernel32` function (the internal shims). Equivalent to
    /// [`extern_idx_dll`](Self::extern_idx_dll) with `"kernel32.dll"`.
    fn extern_idx(&mut self, name: &'static str) -> usize {
        self.extern_idx_dll("kernel32.dll", name)
    }

    /// The import slot for `name` in `dll`, assigning a new one on first reference.
    /// Deduped by function name (each name belongs to one DLL).
    fn extern_idx_dll(&mut self, dll: &'static str, name: &'static str) -> usize {
        if let Some(i) = self.externs.iter().position(|&(_, e)| e == name) {
            return i;
        }
        self.externs.push((dll, name));
        self.externs.len() - 1
    }
    ...
    fn call_aligned(&mut self, asm: &mut Asm, name: &'static str) {
        ...
        asm.mov_rr(R15, RSP); // save rsp
        asm.and_ri(RSP, -16); // 16-align
        asm.emit(&[0x48, 0x83, 0xEC, 0x20]); // sub rsp, 32 (shadow)
        asm.call_extern(i);
        asm.mov_rr(RSP, R15); // restore rsp
    }
    ...
    fn emit_read_write(&mut self, asm: &mut Asm, func: &'static str) {
        ...
        asm.mov_rr(R15, RSP); // save rsp (non-volatile; survives the call)
        asm.and_ri(RSP, -16); // 16-align
...
impl OsTarget for WindowsTarget {
    ...
    fn extern_slot(&mut self, name: &'static str) -> Option<usize> {
        // A `Prim::WinCall` (a Win32 function from `<windows.hc>`) is a direct import.
        // Its DLL comes from the `win_import` table (kernel32 for file I/O, advapi32 for
        // the registry, …). `extern_idx_dll` dedups, so a `WriteFile` WinCall shares the
        // IAT slot with the internal `Write` shim.
        let dll = crate::intrinsics::win_import(name)
            .map(|(_, dll)| dll)
            .unwrap_or("kernel32.dll");
        Some(self.extern_idx_dll(dll, name))
    }
    ...
    fn emit_cpu_ns(&mut self, asm: &mut Asm, sc: i32) {
        ...
        asm.mov_rr(R15, RSP); // save caller rsp (R15 survives the call)
        asm.and_ri(RSP, -16); // 16-align
    ...
    fn wrap(&mut self, asm: Asm, bss: u64) -> Result<Vec<u8>, CodegenError> {
        ...
        // Group the imports by DLL, preserving first-seen DLL order and within-DLL
        // insertion order. Each group becomes one IMAGE_IMPORT_DESCRIPTOR with its own
        // ILT/IAT run (a single kernel32 group reproduces the old single-DLL layout
        // byte-for-byte).
        let mut dll_order: Vec<&'static str> = Vec::new();
        for &(dll, _) in &self.externs {
            if !dll_order.contains(&dll) {
                dll_order.push(dll);
            }
        }
        let groups: Vec<(&'static str, Vec<usize>)> = dll_order
            .iter()
            .map(|&dll| {
                let idxs = self
                    .externs
            ...
            .collect();
        let num_dlls = groups.len();

        // Layout within the import region: the import directory table (one descriptor
        // per DLL plus a null terminator), the import lookup table, the import address
        // table (what the call sites target), then the hint/name table and the DLL
        // names. Each ILT/IAT has `n` name entries plus one NULL terminator per group.
        let idt_size = (num_dlls + 1) * 20;
        let total_thunks = (n + num_dlls) * 8;
        let ilt_off = idt_size;
        let iat_off = ilt_off + total_thunks;
        let hn_off = iat_off + total_thunks;

        // Hint/name entries: a 2-byte hint, the name, and a NUL, padded to an even
        // length. Record each function's RVA (by global slot) for the lookup/address
        // tables.
        let mut hn = Vec::new();
        let mut name_rva = vec![0u32; n];
        let mut cur = hn_off;
        for (i, &(_, name)) in self.externs.iter().enumerate() {
            name_rva[i] = (import_base + cur) as u32;
            put16(&mut hn, 0); // hint
        ...
        // DLL name strings, after the hint/name table; record each group's RVA.
        let mut dll_names = Vec::new();
        let mut dll_name_rva = Vec::with_capacity(num_dlls);
        let mut dcur = hn_off + hn.len();
        for (dll, _) in &groups {
            dll_name_rva.push((import_base + dcur) as u32);
            dll_names.extend_from_slice(dll.as_bytes());
            dll_names.push(0);
            dcur += dll.len() + 1;
        }
        ...
        // Per-group ILT/IAT start entry (groups are laid out back to back, each ending
        // in a NULL), and the global-slot → IAT byte-offset map the call sites use.
        let mut group_start = Vec::with_capacity(num_dlls);
        let mut iat_offsets = vec![0usize; n];
        {
            let mut ec = 0usize;
            for (_dll, idxs) in &groups {
                group_start.push(ec);
                for &gslot in idxs {
                    iat_offsets[gslot] = iat_off + ec * 8;
                    ec += 1;
                }
                ec += 1; // NULL terminator
            }
        }

        let mut import = Vec::new();
        // One IMAGE_IMPORT_DESCRIPTOR per DLL, then the null terminator.
        for (gi, (_dll, _idxs)) in groups.iter().enumerate() {
            put32(&mut import, ilt_rva + (group_start[gi] * 8) as u32); // OriginalFirstThunk
            put32(&mut import, 0); // TimeDateStamp
            put32(&mut import, 0); // ForwarderChain
            put32(&mut import, dll_name_rva[gi]); // Name
            put32(&mut import, iat_rva + (group_start[gi] * 8) as u32); // FirstThunk (IAT)
        }
        import.extend_from_slice(&[0u8; 20]); // null descriptor
        // ILT: each group's name RVAs, NULL-terminated.
        for (_dll, idxs) in &groups {
            for &gslot in idxs {
                put64(&mut import, name_rva[gslot] as u64);
            }
            put64(&mut import, 0);
        }
        // IAT: same shape (the loader overwrites with resolved addresses).
        for (_dll, idxs) in &groups {
            for &gslot in idxs {
                put64(&mut import, name_rva[gslot] as u64);
            }
        ...
        import.extend_from_slice(&dll_names);
