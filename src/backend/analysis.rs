...
//!
//! The operand walkers mirror [`crate::ir::IrInst::for_each_use`] / [`IrTerm::for_each_use`]
...
/// block's id equals its index in [`IrFunc::blocks`]. Works on either SSA or phi-free IR —
...
impl DomTree {
    pub fn new(f: &IrFunc, cfg: &Cfg) -> DomTree {
        let nb = f.blocks.len();
...
impl LoopForest {
    pub fn new(f: &IrFunc, cfg: &Cfg, dom: &DomTree) -> LoopForest {
        let nb = f.blocks.len();
...
/// comes from the canonical [`IrInst::def`]; the register class is derived from the op's
/// result type (a concern specific to register allocation).
pub(crate) fn def_of(i: &IrInst) -> Option<(Vreg, bool)> {
    let dst = i.def()?;
    let is_float = match i {
        IrInst::Bin { ty, .. }
        | IrInst::Un { ty, .. }
        | IrInst::Mov { ty, .. }
        | IrInst::Load { ty, .. } => ty.is_float(),
        IrInst::Cast { to, .. } => to.is_float(),
        IrInst::Call { ret, .. } => matches!(ret, IrRet::Scalar(t) if t.is_float()),
        _ => false,
    };
    Some((dst, is_float))
}

/// Visit each vreg an instruction reads, via the canonical [`IrInst::for_each_use`] (so
/// analyses see exactly the operands the verifier checks — including an indirect callee).
pub(crate) fn uses_of(i: &IrInst, mut f: impl FnMut(Vreg)) {
    i.for_each_use(|val| {
...
/// Visit each vreg a terminator reads, via the canonical [`IrTerm::for_each_use`].
pub(crate) fn term_uses(t: &IrTerm, mut f: impl FnMut(Vreg)) {
    t.for_each_use(|val| {
        if let Some(r) = val.reg() {
            f(r);
        }
