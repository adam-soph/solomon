...
//! [`IrProgram`] directly; the `run_to_string`/`run_to_bytes` entry points at the end of
...
pub(crate) fn cmp(op: CmpOp, ty: IrTy, signed: bool, l: RVal, r: RVal) -> bool {
    if ty.is_float() {
        let a = l.as_f64();
        let b = r.as_f64();
        return match op {
...
// [`IrInterp`] (the conformance oracle). Run semantic analysis and layout first; the
...
pub fn run_to_bytes_with(
    ...
    let mut interp = IrInterp::new(&ir);
