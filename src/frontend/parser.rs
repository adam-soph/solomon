...
/// unrecognized `triple` (e.g. `""`) yields just the `__solomon__` marker.
...
fn parse_core(
    ...
    let (known_types, uses_print, uses_f64tostr) =
        hoist_type_names(src, dir, search, with_prelude, triple)?;
    let mut pp =
...
fn hoist_type_names(
    ...
    // type or print is hoisted exactly as it will be parsed.
    pp = pp.with_target(triple);
    if with_prelude {
