...
impl Analyzer {
            ...
            if let StmtKind::Func(f) = &item.kind {
                ...
                if let Some(existing) = self.funcs.get(&f.name) {
                    if existing.defined && has_body {
                        self.error(item.span.pos, format!("redefinition of function `{}`", f.name));
                        continue;
                    }
    ...
    fn check_declarator(&mut self, d: &Declarator) {
        ...
        if matches!(d.ty, Type::U0) {
            self.error(d.span.pos, format!("variable `{}` cannot have type U0", d.name));
        }
        if let Some(init) = &d.init {
            let vt = self.check_expr(init);
            self.check_assignable(&d.ty, &vt, init.span.pos);
        }
    ...
    fn infer(&mut self, expr: &Expr) -> Type {
        match &expr.kind {
            ...
            ExprKind::Member { base, field, arrow } => self.check_member(base, field, *arrow, expr.span.pos),
        ...
        match op {
            Add | Sub | Mul | Div | Mod => {
                if !is_scalar(&lt) || !is_scalar(&rt) {
                    self.error(lhs.span.pos, "arithmetic requires numeric or pointer operands");
                    return Type::I64;
                }
            ...
            BitAnd | BitOr | BitXor | Shl | Shr => {
                if !is_integer(&lt) || !is_integer(&rt) {
                    self.error(lhs.span.pos, "bitwise/shift operators require integer operands");
                }
                Type::I64
            }
        }
    ...
    fn check_assign(&mut self, _op: AssignOp, target: &Expr, value: &Expr) -> Type {
        ...
        if !self.is_lvalue(target) {
            self.error(target.span.pos, "left-hand side of assignment is not an lvalue");
        }
    ...
    fn check_call(&mut self, callee: &Expr, args: &[Expr]) -> Type {
        ...
        if let ExprKind::Ident(name) = &callee.kind {
            ...
            // No matching function: a genuine error. (Core/standard-library
            // functions will be registered here once those exist.)
            self.error(
                callee.span.pos,
        ...
        // Calling a computed value (e.g. a function pointer): check it, assume
        // I64 result.
